...
type CompletedSectionLike = DocumentSectionLike & {
  content: string;
};
...
const assembleDocumentHandler = async (
  ctx: any,
  ...
  try {
    ...
    const sortedSections: DocumentSectionLike[] = [...document.sections].sort(
      (a: DocumentSectionLike, b: DocumentSectionLike) => {
    ...
    const completedSections: CompletedSectionLike[] = sortedSections.filter(
      (s: DocumentSectionLike): s is CompletedSectionLike =>
        s.status === "complete" && typeof s.content === "string" && s.content.length > 0,
    );
    ...
    const sectionsList = completedSections.map((s: CompletedSectionLike) => ({
    ...
    logStep(
      2,
      4,
      `Front matter with ${completedSections.length} TOC entries`,
      "success",
    );
    ...
    for (const section of completedSections) {
      ...
      const normalizedHeaderTitle = headerMatch?.[1]?.toLowerCase().trim() ?? "";
      const contentStartsWithTitle = normalizedHeaderTitle.includes(
...
export const assembleDocument = internalAction({
  ...
  handler: assembleDocumentHandler,
});
